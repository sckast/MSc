...
focal.species <- "Dryas octopetala"
...
points.with.trees.dryas <- focal.co.long %>%
  filter(grepl('Abies|Picea|Pinus|Larix|Tsuga', species)) %>%
  group_by(focal_gbifID) %>%
  count(species) %>%
  arrange(desc(n))
...
points.of.interest.dryas.trees <- sf.focal.essf.alpine.south %>%
  filter(as.character(gbifID) %in% points.with.trees.dryas$focal_gbifID)

ggplot() + geom_sf(data = bc.boundary, fill = NA) +
  geom_sf(data = points.of.interest.dryas.trees, colour = "red", shape = 1) +
  coord_sf(crs = st_crs(points.of.interest.dryas.trees), 
           xlim = c(-130, -112), ylim = c(48, 53)) +
  theme_bw()

points.of.interest.dryas.trees.csv <- st_drop_geometry(points.of.interest.dryas.trees)

# write to csv

readr::write_csv(freq.species, "species_frequency_dryas.csv")
readr::write_csv(point.richness, "point_richness_dryas.csv")
readr::write_csv(points.of.interest.dryas.trees.csv, 
                 "points_of_interest_dryas_trees.csv")

# export KML for viewing in Google Earth

st_write(points.of.interest.dryas.trees, "points_of_interest_dryas_trees.kml", 
